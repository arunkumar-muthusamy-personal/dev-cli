...
VERSION_PY = ROOT / "src/dev_cli/version.py"
...
def set_version_in_toml(new: str) -> None:
    text = TOML.read_text(encoding="utf-8")
    updated = re.sub(
        r'^(version\s*=\s*)"[^"]+"',
        f'\\g<1>"{new}"',
...
def set_version_in_version_py(new: str) -> None:
...
def main() -> None:
    ...
    set_version_in_toml(new_version)
    print(f"✅ Updated pyproject.toml → {new_version}")
                                                                                                                        
    # 2. Update version.py
    set_version_in_version_py(new_version)
    print(f"✅ Updated version.py → {new_version}")
                                                                                                                        
    # 3. Stage + commit
    run(f'git add pyproject.toml src/dev_cli/version.py')
